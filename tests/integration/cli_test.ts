...
import { runCli, VERSION } from '../../src/cli/cli.ts';
import {
  loadTestMacroPackageFiles,
  TEST_MACRO_PACKAGE_NAME,
} from '../support/test_macro_package_fixture.ts';
import {
...
async function loadRuntimeReferencesStackOverflowFixture(): Promise<string> {
  return await Deno.readTextFile(
    new URL(
      '../fixtures/runtime-references-stack-overflow/runtime-references.sts',
      import.meta.url,
    ),
  );
}
...
Deno.test('runCli prints help text', async () => {
  ...
  assertStringIncludes(result.output, 'node');
...
Deno.test('runCli node materializes a mixed .ts/.sts app through a temporary transformed graph', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify(
  ...
  const result = await runCli(
    ...
    tempDirectory,
    {
      runSubprocess: async (command, args, cwd) => {
        seenCommand = command;
        seenArgs = [...args];
        seenCwd = cwd;
        ...
        assertStringIncludes(runtimePackageJson, '"type": "module"');
        return {
          exitCode: 0,
          output: '42\n',
        };
  ...
  assertEquals(result.exitCode, 0);
  assertEquals(seenCommand, 'node');
  assertEquals(seenCwd, tempDirectory);
  assertEquals(seenArgs[0], '--enable-source-maps');
...
Deno.test('runCli node materializes access to installed runtime packages for temp graph execution', async () => {
  const tempDirectory = await createTempProject([
    ...
    {
      path: 'node_modules/@soundscript/soundscript/package.json',
      contents: JSON.stringify(
        {
    ...
    {
      path: 'node_modules/@soundscript/soundscript/index.js',
      contents: 'export const ok = (value) => ({ tag: "ok", value });\n',
    },
    {
      path: 'node_modules/@soundscript/soundscript/index.d.ts',
      contents: [
  ...
  const result = await runCli(
    ['node', join(tempDirectory, 'src/main.sts')],
    tempDirectory,
    {
      runSubprocess: async (_command, args) => {
        ...
        const linkedRuntimePackageJson = await Deno.readTextFile(
          join(runtimeRoot, 'node_modules/@soundscript/soundscript/package.json'),
        );
        const entryText = await Deno.readTextFile(args[1]!);

        assertStringIncludes(runtimePackageJson, '"type": "module"');
        assertStringIncludes(linkedRuntimePackageJson, '"name": "@soundscript/soundscript"');
        assertStringIncludes(entryText, "from '@soundscript/soundscript'");
        return {
          exitCode: 0,
          output: 'ok\n',
        };
  ...
  assertEquals(result.exitCode, 0);
  assertStringIncludes(result.output, 'ok');
});

Deno.test('runCli node emits inline source maps back to original .sts source', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify(
  ...
  const result = await runCli(
    ...
    tempDirectory,
    {
      runSubprocess: async (_command, args) => {
        const entryText = await Deno.readTextFile(args[1]!);
        assertStringIncludes(entryText, "throw new Error('boom');");
        const sourceMapPrefix = '//# sourceMappingURL=data:application/json;base64,';
        assertStringIncludes(entryText, sourceMapPrefix);
        const encodedMap = entryText.slice(
          entryText.lastIndexOf(sourceMapPrefix) + sourceMapPrefix.length,
        ).trim();
        const decodedMap = new TextDecoder().decode(
          Uint8Array.from(atob(encodedMap), (character) => character.charCodeAt(0)),
        );
        assertStringIncludes(decodedMap, '/src/main.sts');
        assertStringIncludes(decodedMap, "throw new Error('boom');");
        return {
          exitCode: 1,
          output: 'Error: boom\n    at /virtual/src/main.sts:1:7\n',
        };
      },
    },
  );
  ...
  assertStringIncludes(result.output, 'src/main.sts');
});
...
Deno.test('runCli editor-project prints projected editor json for one sts file', async () => {
  const tempDirectory = await createTempProject([
    ...
    },
    {
      ...
      contents: [
        '// #[interop]',
        "import { type Environment, value } from '../../src/types.ts';",
