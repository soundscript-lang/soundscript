...
function createArrayCallbackMatrixCallbackParameters(
  ...
  if (isArrayCallbackMatrixReduceMethod(method)) {
    return route === 'elementParameter'
      ...
      : `${withType('acc', 'number')}, ${withType('_current', 'Obj')}, ${
        withType('_index', 'number')
      }, ${withType('items', 'Obj[]')}`;
  }

  return route === 'elementParameter'
    ? withType('current', 'Obj')
    : `${withType('_current', 'Obj')}, ${withType('_index', 'number')}, ${
      withType('items', 'Obj[]')
    }`;
}
...
function createArrayCallbackMatrixCallbackSource(
  ...
  const parameters = createArrayCallbackMatrixCallbackParameters(
    method,
    route,
...
function createArrayCallbackMatrixFixture(
  ...
  const routeLabel = route === 'elementParameter' ? 'the element parameter' : 'the array parameter';
...
function createArrayCallbackMatrixFixtures(): readonly FixtureCase[] {
  ...
  const forms: readonly ArrayCallbackMatrixForm[] = [
    'inline',
    'localIdentifier',
    'importedIdentifier',
  ];
  const routes: readonly ArrayCallbackMatrixMutationRoute[] = [
    'elementParameter',
    'arrayParameter',
  ];
...
function createPromiseThenCallbackMatrixCallbackSource(
  form: PromiseThenCallbackMatrixForm,
): string {
  if (form === 'inline') {
    return `(current) => {
      current.value = 42;
...
function createPromiseThenCallbackMatrixFixtures(): readonly FixtureCase[] {
  const forms: readonly PromiseThenCallbackMatrixForm[] = [
    'inline',
    'localIdentifier',
    'importedIdentifier',
  ];
...
function createPromiseThenScheduledCallbackFixture(
  form: PromiseThenCallbackMatrixForm,
): FixtureCase {
  const formSlug = form === 'inline'
    ? 'inline'
