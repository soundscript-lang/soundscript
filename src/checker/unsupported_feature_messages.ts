  ...
  switch (kind) {
    ...
    case 'argumentsCallee':
      return createUnsupportedFeature(kind, '`arguments.callee` is not supported in soundscript.', {
        ...
        example:
          'Write `function visit() { return visit(); }` instead of reading `arguments.callee`.',
        invariant:
    ...
    case 'argumentsObject':
      return createUnsupportedFeature(
        kind,
        'The `arguments` object is not supported in soundscript.',
        {
    ...
    case 'arrayLengthConstructor':
      return createUnsupportedFeature(
        ...
        'Length-only `Array(...)` construction is not supported in soundscript.',
        {
          ...
          invariant:
            'Arrays must be constructed with an explicit element story rather than implicit holes.',
    ...
    case 'bannedConstructor': {
      ...
      return createUnsupportedFeature(
        ...
        `Constructing \`${constructorName}\` is not supported in soundscript.`,
        {
          ...
          invariant:
            'Only explicitly modeled runtime constructors are available inside soundscript.',
    ...
    case 'bannedDeclarationFileTypeReference': {
      ...
      return createUnsupportedFeature(
        kind,
        `Type reference \`${typeName}\` is not supported in soundscript.`,
        {
          hint:
    ...
    case 'classInterfaceMerge':
      return createUnsupportedFeature(
        kind,
        'Class/interface merging is not supported in soundscript.',
        {
    ...
    case 'debuggerStatement':
      return createUnsupportedFeature(
        kind,
        '`debugger` statements are not supported in soundscript.',
        {
    ...
    case 'deleteExpression':
      return createUnsupportedFeature(kind, '`delete` is not supported in soundscript.', {
        ...
        invariant:
          'Object shape changes must stay explicit instead of mutating property existence at runtime.',
    ...
    case 'disallowedThis':
      return createUnsupportedFeature(kind, '`this` is not supported here in soundscript.', {
        hint:
          'Pass the value explicitly, or move the logic into an instance method or constructor.',
        replacementFamily: 'explicit_receiver_parameter',
      });
    case 'eval':
      return createUnsupportedFeature(kind, '`eval` is not supported in soundscript.', {
        hint:
          'Use explicit function calls, parsers, or interpreters instead of dynamic code execution.',
    ...
    case 'functionConstructor':
      return createUnsupportedFeature(
        kind,
        'The `Function` constructor is not supported in soundscript.',
        {
          hint:
            'Write a real function or parser instead of compiling code from strings at runtime.',
    ...
    case 'incompatibleStrictEquality':
      return createUnsupportedFeature(
        ...
        'Strict equality in soundscript only compares values from the same primitive family (aside from nullish checks).',
        {
          ...
          invariant:
            'Equality comparisons must stay inside one primitive family unless they are nullish checks.',
    ...
    case 'labeledStatement':
      return createUnsupportedFeature(
        kind,
        'Labeled statements are not supported in soundscript.',
        {
    ...
    case 'legacyAccessorApi':
      return createUnsupportedFeature(
        ...
        'Legacy accessor APIs like `__defineGetter__` are not supported in soundscript.',
        {
          ...
          invariant:
            'Accessor behavior must be visible in the declared surface instead of patched at runtime.',
          replacementFamily: 'explicit_methods_or_fields',
        },
      );
    ...
    case 'looseEquality':
      return createUnsupportedFeature(
        ...
        'Loose equality (`==` / `!=`) is not supported in soundscript.',
        {
          ...
          example:
            'Write `value === null` or `Number(text) === count` instead of relying on `==` coercion.',
    ...
    case 'nonBooleanCondition':
      return createUnsupportedFeature(
        kind,
        'Conditions in soundscript must be boolean expressions.',
        {
    ...
    case 'nonBooleanLogicalNot':
      return createUnsupportedFeature(
        kind,
        'Logical `!` in soundscript requires a boolean operand.',
        {
          ...
          invariant:
            'Logical negation must invert an explicit boolean condition, not JavaScript truthiness.',
    ...
    case 'nonBooleanLogicalOperator':
      return createUnsupportedFeature(
        ...
        'Logical `&&` and `||` in soundscript require boolean operands.',
        {
          ...
          example:
            'Write `isReady && count > 0` instead of `value && count` when every operand should be boolean.',
          invariant:
            'Logical operators must compose boolean conditions rather than value-carrying truthiness.',
    ...
    case 'objectPrimitiveBoxing':
      return createUnsupportedFeature(
        kind,
        '`Object(...)` cannot be used to box primitives in soundscript.',
        {
    ...
    case 'primitiveConversionHookCall':
      return createUnsupportedFeature(
        ...
        'Calling `.toString()` or `.valueOf()` on arbitrary values is not supported in soundscript.',
        {
          ...
          invariant:
            'Primitive conversion must be explicit and type-directed instead of hook-driven.',
    ...
    case 'protoProperty':
      return createUnsupportedFeature(kind, '`__proto__` is not supported in soundscript.', {
        hint:
          'Use ordinary objects, `BareObject`, or factory functions instead of mutating prototypes.',
        example:
    ...
    case 'prototypeMutation':
      return createUnsupportedFeature(
        kind,
        "Changing an object's prototype is not supported in soundscript.",
        {
          hint:
            'Use ordinary objects, `BareObject`, or explicit factory functions instead of mutating prototypes.',
          example:
    ...
    case 'proxyRevocable':
      return createUnsupportedFeature(
        kind,
        '`Proxy.revocable(...)` is not supported in soundscript.',
        {
    ...
    case 'reflectiveMetaObjectOperation':
      return createUnsupportedFeature(
        ...
        'Reflective property-definition and own-keys APIs are not supported in soundscript.',
        {
          ...
          invariant:
            'Object shape must come from explicit declarations rather than runtime meta-object surgery.',
    ...
    case 'scriptScopeInterfaceMerge':
      return createUnsupportedFeature(
        kind,
        'Script-scope interface merging is not supported in soundscript.',
        {
          hint:
    ...
    case 'sparseArrayLiteral':
      return createUnsupportedFeature(
        kind,
        'Sparse array literals are not supported in soundscript.',
        {
    ...
    case 'symbolApi':
      return createUnsupportedFeature(
        kind,
        '`Symbol` and `Symbol.for(...)` are not supported in soundscript.',
        {
    ...
    case 'symbolHook':
      return createUnsupportedFeature(
        kind,
        'Custom `Symbol.*` protocol hooks are not supported in soundscript.',
        {
    ...
    case 'templateInterpolation':
      return createUnsupportedFeature(
        kind,
        'Template literal interpolations in soundscript must already be `string`.',
        {
    ...
    case 'withStatement':
      return createUnsupportedFeature(kind, '`with` statements are not supported in soundscript.', {
        hint:
          'Read the properties you need into locals explicitly instead of changing name lookup rules.',
