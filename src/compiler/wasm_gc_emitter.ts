...
function renderStringSearchExpression(
  ...
  const sk = expression.searchKind;
  const searchKindIdx = sk === 'startsWith' ? '0' : sk === 'endsWith' ? '1'
    : sk === 'includes' ? '2' : sk === 'indexOf' ? '3' : '4';
  const result = [
    ...renderExpression(expression.value, indent, context),
    ...renderExpression(expression.search, indent, context),
  ...
  if (sk === 'indexOf' || sk === 'lastIndexOf') {
    result.push(`${indent}f64.convert_i32_s`);
  }
  return result;
}
...
function renderStringSearchHelperFunctions(plan: WasmGcModulePlanIR): readonly string[] {
  ...
  const funcName = sanitizeIdentifier(STRING_SEARCH_FUNCTION_NAME);
  const st = stringRuntimeTypeName();
  const ca = stringCodeUnitArrayTypeName();
  return [
    `  (func $${funcName} (param $value (ref null ${st})) (param $search (ref null ${st})) (param $kind i32) (result i32)`,
    `    (local $value_units (ref null ${ca}))`,
    `    (local $search_units (ref null ${ca}))`,
    `    (local $vlen i32)`,
    `    (local $slen i32)`,
    `    (local $i i32)`,
    `    (local $j i32)`,
    ...
    `    local.set $value_units`,
    `    local.get $search`,
    `    ref.as_non_null`,
    `    struct.get $${st} $code_units`,
    `    local.set $search_units`,
    `    local.get $value_units`,
    `    array.len`,
    `    local.set $vlen`,
    `    local.get $search_units`,
    `    array.len`,
    `    local.tee $slen`,
    `    local.get $vlen`,
    `    i32.gt_u`,
    ...
    `      return`,
    `    end`,
    `    ;; startsWith: single check at offset 0`,
    `    i32.const 0`,
    `    local.set $offset`,
    `    i32.const 0`,
    `    local.set $i`,
    ...
    `    i32.const 0`,
    `    return`,
    `    ;; endsWith: single check at offset vlen-slen`,
    `  end`,
    `    local.get $vlen`,
    `    local.get $slen`,
    `    i32.sub`,
    ...
    `    i32.const 0`,
    `    return`,
    `    ;; indexOf: loop offsets 0..max, return offset on match, -1 on no match`,
    `  end`,
    `    i32.const 0`,
    `    local.set $offset`,
    `    local.get $vlen`,
    ...
    `    i32.const -1`,
    `    return`,
    `    ;; lastIndexOf: loop offsets max..0, return offset on match, -1 on no match`,
    `  end`,
    `    local.get $vlen`,
    `    local.get $slen`,
    `    i32.sub`,
...
function renderStringSplitHelperFunctions(plan: WasmGcModulePlanIR): readonly string[] {
  ...
  const st = stringRuntimeTypeName();
  const ca = stringCodeUnitArrayTypeName();
  const sr = 'string_array_runtime';
  return [
    `  (func $${n} (param $value (ref null ${st})) (param $sep (ref null ${st})) (result (ref null ${sr}))`,
    `    (local $units (ref null ${ca}))`,
    `    (local $len i32)`,
    `    (local $sep_units (ref null ${ca}))`,
    `    (local $sep_len i32)`,
    `    (local $result (ref null ${sr}))`,
    `    (local $seg_start i32)`,
    `    (local $i i32)`,
    `    (local $count i32)`,
    `    (local $seg (ref null ${st}))`,
    `    (local $seg_units (ref null ${ca}))`,
    `    (local $j i32)`,
    `    (local $seg_len i32)`,
    `    (local $match i32)`,
    `    (local $k i32)`,
...
function renderStringReplaceHelperFunctions(plan: WasmGcModulePlanIR): readonly string[] {
  ...
  const ca = stringCodeUnitArrayTypeName();
  return [
    `  (func $${n} (param $value (ref null ${st})) (param $search (ref null ${st})) (param $rep (ref null ${st})) (param $all i32) (result (ref null ${st}))`,
    `    (local $units (ref null ${ca}))`,
    `    (local $len i32)`,
    `    (local $srch_units (ref null ${ca}))`,
    `    (local $srch_len i32)`,
    `    (local $rep_units (ref null ${ca}))`,
    `    (local $rep_len i32)`,
    `    (local $result_units (ref null ${ca}))`,
    `    (local $result_len i32)`,
    `    (local $result (ref null ${st}))`,
...
function renderStringPadHelperFunctions(plan: WasmGcModulePlanIR): readonly string[] {
  ...
  const ca = stringCodeUnitArrayTypeName();
  return [
    `  (func $${n} (param $value (ref null ${st})) (param $target i32) (param $pad (ref null ${st})) (param $side i32) (result (ref null ${st}))`,
    `    (local $units (ref null ${ca}))`,
    `    (local $len i32)`,
    `    (local $pad_cu i32)`,
    `    (local $result_units (ref null ${ca}))`,
