...
compilerIntegrationTest(
  'compileProject adapts exported Map results with union payloads through the JS boundary',
  async () => {
    const tempDirectory = await createTempProject([
      {
        path: 'tsconfig.json',
      ...
      },
      {
        path: 'src/index.ts',
        contents: [
          'export function main(): Map<string, string | number> {',
          '  const values = new Map<string, string | number>();',
          '  values.set("left", 4);',
          '  values.set("right", "abc");',
          '  return values;',
          '}',
          '',
        ].join('\n'),
    ...
    assertEquals([...values.entries()], [['left', 4], ['right', 'abc']]);
  },
);

compilerIntegrationTest(
  'compileProject adapts exported Set results with union payloads through the JS boundary',
  async () => {
    const tempDirectory = await createTempProject([
      {
        path: 'tsconfig.json',
      ...
      },
      {
        path: 'src/index.ts',
        contents: [
          'export function main(): Set<string | number> {',
    ...
    assertEquals([...values.values()], [4, 'abc']);
  },
);
...
compilerIntegrationTest(
  'compileProject emits a WasmGC-backed public wrapper for wasm-node string boundaries',
  async () => {
    const tempDirectory = await createTempProject([
      {
        path: 'tsconfig.json',
    ...
    const instantiated = await wrapperModule.instantiate();
    const exportName = await resolveQualifiedExportName(tempDirectory, 'echo');
    const exported = instantiated.exports[exportName];
    if (typeof exported !== 'function') {
      throw new Error(`Expected exported function "${exportName}".`);
    }
    assertEquals(exported('A😀'), 'A😀');
  },
);
...
compilerIntegrationTest(
  'compileProject emits a WasmGC-backed public wrapper for wasm-node modules with internal object layouts',
  async () => {
    const tempDirectory = await createTempProject([
      {
        path: 'tsconfig.json',
...
compilerIntegrationTest(
  'compileProject emits a WasmGC-backed public wrapper for wasm-node fixed-layout object results',
  async () => {
    const tempDirectory = await createTempProject([
      {
        path: 'tsconfig.json',
...
compilerIntegrationTest(
  'compileProject copies back wasm-node map-set-valued object param mutations through the WasmGC public wrapper',
  async () => {
    const tempDirectory = await createTempProject([
      {
        path: 'tsconfig.json',
      ...
      },
      {
        ...
        contents: [
          ...
          'export function update(bag: Bag): Bag {',
          '  const left = bag.values.get("left");',
          '  bag.values.set("left", left === undefined ? 1 : left + 1);',
          '  bag.flags.add("bee");',
          '  return bag;',
          '}',
          '',
    ...
    const returned = exported(bag);
    assertStrictEquals(returned, bag);
    assertStrictEquals(bag.values, values);
    assertStrictEquals(bag.flags, flags);
    assertEquals([...bag.values.entries()], [['left', 5], ['right', 2]]);
    assertEquals([...bag.flags.values()], ['ant', 'bee']);
  },
);
    ...
    const tempDirectory = await createTempProject([
      ...
      },
      {
        ...
        contents: [
          'type Pair = { left: number; right: number };',
          '',
          'declare function bump(pair: Pair): Pair;',
          '',
          'export function main(): number {',
          '  const pair = bump({ left: 3, right: 4 });',
          '  return pair.left * 10 + pair.right;',
          '}',
          '',
        ].join('\n'),
    ...
    const instance = await instantiateCompiledModuleInJs(tempDirectory, {
      hostFunctions: {
        'src/index.ts:bump': (input: unknown) => {
          const pair = input as { left: number; right: number };
          pair.left += 2;
          return pair;
        },
      },
...
compilerIntegrationTest(
  'compileProject adapts wasm-node object-valued union results through the WasmGC public wrapper',
  async () => {
    const tempDirectory = await createTempProject([
      {
        path: 'tsconfig.json',
    ...
    assertEquals(exported(false), { right: true });
...
compilerIntegrationTest(
  'compileProject adapts wasm-node nested collection union fields through the WasmGC public wrapper',
  async () => {
    const tempDirectory = await createTempProject([
      {
        path: 'tsconfig.json',
      ...
      },
      {
        ...
        contents: [
          ...
          '};',
          '',
          'export function make(flag: boolean): Bag {',
          '  if (flag) {',
          ...
          '    flags.add(3);',
          '    flags.add(5);',
          '    return { values: [2, 8], scores, flags };',
          '  }',
          '  return { values: null, scores: null, flags: null };',
          '}',
          '',
          ...
          '  const valuesScore = values === null ? 0 : values[0] + values[1];',
          '  const mapScore = scores === null ? 0 : (scores.get("left") ?? 0) + (scores.get("right") ?? 0);',
          '  const setScore = flags === null ? 0 : (flags.has(3) ? 10 : 0) + (flags.has(5) ? 1 : 0);',
    ...
    const instance = await instantiateCompiledModuleInJs(tempDirectory);
    const makeName = await resolveQualifiedExportName(tempDirectory, 'make');
    const make = instance.exports[makeName];
    if (typeof make !== 'function') {
      throw new Error(`Expected exported function "${makeName}".`);
    }
    const positive = make(true) as {
      values: number[];
      scores: Map<string, number>;
      flags: Set<number>;
    };
    assertEquals(positive.values, [2, 8]);
    assertEquals([...positive.scores.entries()], [['left', 4], ['right', 6]]);
    assertEquals([...positive.flags.values()], [3, 5]);
    assertEquals(make(false), { values: null, scores: null, flags: null });
...
compilerIntegrationTest(
  'compileProject imports host array members of mixed tagged unions',
  async () => {
    const tempDirectory = await createTempProject([
      {
        path: 'tsconfig.json',
      ...
      },
      {
        ...
        contents: [
          ...
          '  export type Value = number | string | string[] | Left;',
...
compilerIntegrationTest(
  'compileProject passes host array members of mixed tagged unions',
  async () => {
    const tempDirectory = await createTempProject([
      {
        path: 'tsconfig.json',
...
compilerIntegrationTest(
  'compileProject exports host array members of mixed tagged unions',
  async () => {
    const tempDirectory = await createTempProject([
      {
        path: 'tsconfig.json',
    ...
    assertEquals((exported(3) as { left: number }).left, 5);
...
compilerIntegrationTest(
  'compileProject accepts host array members of mixed tagged union exports',
  async () => {
    const tempDirectory = await createTempProject([
      {
        path: 'tsconfig.json',
...
compilerIntegrationTest(
  'compileProject imports host callable members of mixed tagged unions',
  async () => {
    const tempDirectory = await createTempProject([
      {
        path: 'tsconfig.json',
      ...
      },
      {
        ...
        contents: [
          ...
          '  export type Value = number | string | Compute | Left;',
          '  export function next(index: number): Value;',
          '}',
          '',
        ].join('\n'),
      },
      {
        ...
        contents: [
          ...
          'type Left = { left: number };',
          'type Compute = (base: number) => number;',
          'type Value = number | string | Compute | Left;',
          '',
          'function score(value: Value): number {',
          '  if (typeof value === "function") {',
          '    return value(4);',
...
compilerIntegrationTest(
  'compileProject passes host callable members of mixed tagged unions',
  async () => {
    const tempDirectory = await createTempProject([
      {
        path: 'tsconfig.json',
...
compilerIntegrationTest(
  'compileProject exports host callable members of mixed tagged unions',
  async () => {
    const tempDirectory = await createTempProject([
      {
        path: 'tsconfig.json',
...
compilerIntegrationTest(
  'compileProject accepts host callable members of mixed tagged union exports',
  async () => {
    const tempDirectory = await createTempProject([
      {
        path: 'tsconfig.json',
