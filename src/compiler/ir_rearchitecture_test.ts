...
Deno.test('compileProject selects source-hir for for-of over Map entries', async () => {
  const tempDirectory = await createTempProject([
...
Deno.test('compiler SourceHIR semantic lowering emits runnable closure parameter calls', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
    ...
    },
    {
      path: 'main.ts',
      contents: `
        function apply(fn: (value: number) => number, value: number): number {
          return fn(value);
        }
  ...
  const manifest = createRuntimeManifestFromSemanticModule(semantic);
  const plan = createWasmGcModulePlan(semantic, manifest);
  const applyPlan = plan.functionPlans.find((func) => func.name === 'apply');
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const closurePlan = plan.functionPlans.find((func) => func.closureFunctionId !== undefined);
  const watPath = join(tempDirectory, 'wasm-gc-source-closure-param.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-closure-param.wasm');
  ...
  const score = instance.instance.exports['main.ts:score'];
  assertEquals(typeof score, 'function');
  assertEquals((score as () => number)(), 42);
});

Deno.test('compiler SourceHIR semantic lowering emits runnable captured closures through parameters', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const closurePlan = plan.functionPlans.find((func) => func.closureFunctionId !== undefined);
  const watPath = join(tempDirectory, 'wasm-gc-source-captured-closure-param.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-captured-closure-param.wasm');
...
Deno.test('compiler SourceHIR semantic lowering emits runnable closure function results', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
    ...
    },
    {
      ...
      contents: `
        function make(offset: number): (value: number) => number {
          ...
          const add = (value: number): number => value + captured;
          return add;
        }
  ...
  const manifest = createRuntimeManifestFromSemanticModule(semantic);
  const plan = createWasmGcModulePlan(semantic, manifest);
  const makePlan = plan.functionPlans.find((func) => func.name === 'make');
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const closurePlan = plan.functionPlans.find((func) => func.closureFunctionId !== undefined);
  const watPath = join(tempDirectory, 'wasm-gc-source-closure-result.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-closure-result.wasm');

  assertEquals(
    manifest.familyRequirements.map((requirement) => requirement.family),
    ['closure'],
  );
  assertEquals(makePlan?.bodyStatus, 'emittable');
  assertEquals(scorePlan?.bodyStatus, 'emittable');
  assertEquals(closurePlan?.bodyStatus, 'emittable');
  await Deno.writeTextFile(watPath, emitWasmGcModulePlan(plan));
  const wat = await Deno.readTextFile(watPath);
...
Deno.test('compiler SourceHIR semantic lowering emits runnable direct closure result calls', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const closurePlan = plan.functionPlans.find((func) => func.closureFunctionId !== undefined);
  const watPath = join(tempDirectory, 'wasm-gc-source-direct-closure-result.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-direct-closure-result.wasm');
...
Deno.test('compiler SourceHIR semantic lowering emits runnable class field construction', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-class-field.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-class-field.wasm');
  ...
  await Deno.writeTextFile(watPath, emitWasmGcModulePlan(plan));
  const wat = await Deno.readTextFile(watPath);
  assertEquals(wat.includes('struct.new $object_layout_source_object_value_f64'), true);
  assertEquals(wat.includes('struct.set $object_layout_source_object_value_f64 $value'), true);
  assertEquals(wat.includes('struct.get $object_layout_source_object_value_f64 $value'), true);
  const result = await new Deno.Command('wasm-tools', {
    args: ['parse', watPath, '-o', wasmPath],
    stdout: 'piped',
    stderr: 'piped',
...
Deno.test('compiler SourceHIR semantic lowering emits runnable direct class method calls', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-class-method.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-class-method.wasm');
  ...
  const score = instance.instance.exports['main.ts:score'];
  assertEquals(typeof score, 'function');
  assertEquals((score as () => number)(), 82);
});

Deno.test('compiler SourceHIR semantic lowering emits runnable class method params and control flow', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-class-method-params.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-class-method-params.wasm');
...
Deno.test('compiler SourceHIR semantic lowering emits runnable static class member reads and calls', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-class-static.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-class-static.wasm');
  ...
  const wat = await Deno.readTextFile(watPath);
  assertEquals(wat.includes('(type $closure_object (struct'), false);
  assertEquals(wat.includes('global'), false);
  const result = await new Deno.Command('wasm-tools', {
    args: ['parse', watPath, '-o', wasmPath],
    stdout: 'piped',
    stderr: 'piped',
  ...
  const score = instance.instance.exports['main.ts:score'];
  assertEquals(typeof score, 'function');
  assertEquals((score as () => number)(), 44);
});

Deno.test('compiler SourceHIR semantic lowering emits runnable constructor aliases', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-constructor-alias.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-constructor-alias.wasm');
  ...
  await Deno.writeTextFile(watPath, emitWasmGcModulePlan(plan));
  const wat = await Deno.readTextFile(watPath);
  assertEquals(wat.includes('class_constructor'), false);
  assertEquals(wat.includes('struct.new $object_layout_source_object_value_f64'), true);
  const result = await new Deno.Command('wasm-tools', {
    args: ['parse', watPath, '-o', wasmPath],
    stdout: 'piped',
...
Deno.test('compiler SourceHIR semantic lowering emits runnable constructor shadowed parameters', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-constructor-shadowed-param.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-constructor-shadowed-param.wasm');
  ...
  const score = instance.instance.exports['main.ts:score'];
  assertEquals(typeof score, 'function');
  assertEquals((score as (value: number) => number)(41), 42);
});

Deno.test('compiler SourceHIR semantic lowering emits runnable this method calls', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-this-method-call.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-this-method-call.wasm');
...
Deno.test('compiler SourceHIR semantic lowering emits runnable class instance aliases', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-class-instance-alias.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-class-instance-alias.wasm');
...
Deno.test('compiler SourceHIR semantic lowering emits runnable direct new-expression property reads', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-new-expression-property.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-new-expression-property.wasm');
...
Deno.test('compiler SourceHIR semantic lowering emits runnable direct new-expression method calls', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-new-expression-method.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-new-expression-method.wasm');
...
Deno.test('compiler SourceHIR semantic lowering emits runnable class instance parameters', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-class-instance-param.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-class-instance-param.wasm');
...
Deno.test('compiler SourceHIR semantic lowering emits runnable class instance function results', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
    ...
    },
    {
      ...
      contents: `
        ...
        function make(value: number): Counter {
          return new Counter(value);
        }

        export function score(): number {
          return make(41).value + 1;
        }
      `,
    },
  ]);
  ...
  const makePlan = plan.functionPlans.find((func) => func.name === 'make');
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-class-instance-result.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-class-instance-result.wasm');
...
Deno.test('compiler SourceHIR semantic lowering emits runnable method calls on class instance results', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const makePlan = plan.functionPlans.find((func) => func.name === 'make');
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-class-instance-result-method.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-class-instance-result-method.wasm');
...
Deno.test('compiler SourceHIR semantic lowering catches direct builtin Error throws', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-catch-error.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-catch-error.wasm');
...
Deno.test('compiler SourceHIR semantic lowering reads caught builtin Error messages', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-caught-error-message.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-caught-error-message.wasm');
  ...
  const score = instance.instance.exports['main.ts:score'];
  assertEquals(typeof score, 'function');
  assertEquals((score as () => number)(), 4);
});

Deno.test('compiler SourceHIR semantic lowering projects caught builtin Error bindings into object params', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
    ...
    },
    {
      path: 'main.ts',
      contents: `
        function messageLength(error: { message: string }): number {
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-caught-error-object-param.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-caught-error-object-param.wasm');

  assertEquals(
    manifest.familyRequirements.map((requirement) => requirement.family),
    ['dynamic_object', 'error', 'finite_union', 'specialized_object', 'string'],
  );
  assertEquals(scorePlan?.bodyStatus, 'emittable');
  await Deno.writeTextFile(watPath, emitWasmGcModulePlan(plan));
  const result = await new Deno.Command('wasm-tools', {
...
Deno.test('compiler SourceHIR semantic lowering runs finally after caught Error throws', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-caught-error-finally.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-caught-error-finally.wasm');
...
Deno.test('compiler SourceHIR semantic lowering runs nested finally before caught throws', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-nested-try-finally-throw.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-nested-try-finally-throw.wasm');
...
Deno.test('compiler SourceHIR semantic lowering runs finally before catch rethrows', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-catch-rethrow-finally.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-catch-rethrow-finally.wasm');
  ...
  const score = instance.instance.exports['main.ts:score'];
  assertEquals(typeof score, 'function');
  assertEquals((score as () => number)(), 18);
});

Deno.test('compiler SourceHIR semantic lowering returns from plain try blocks', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-try-return-catch.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-try-return-catch.wasm');
...
Deno.test('compiler SourceHIR semantic lowering returns from plain catch blocks', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-catch-return.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-catch-return.wasm');
  ...
  const score = instance.instance.exports['main.ts:score'];
  assertEquals(typeof score, 'function');
  assertEquals((score as () => number)(), 9);
});

Deno.test('compiler SourceHIR semantic lowering breaks from plain try blocks', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-try-break-catch.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-try-break-catch.wasm');
  ...
  const score = instance.instance.exports['main.ts:score'];
  assertEquals(typeof score, 'function');
  assertEquals((score as () => number)(), 1);
});

Deno.test('compiler SourceHIR semantic lowering continues from plain catch blocks', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-catch-continue.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-catch-continue.wasm');
  ...
  const score = instance.instance.exports['main.ts:score'];
  assertEquals(typeof score, 'function');
  assertEquals((score as () => number)(), 3);
});

Deno.test('compiler SourceHIR semantic lowering runs finally before catch returns', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const makePlan = plan.functionPlans.find((func) => func.name === 'make');
  const watPath = join(tempDirectory, 'wasm-gc-source-catch-return-finally.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-catch-return-finally.wasm');
...
Deno.test('compiler SourceHIR semantic lowering runs finally before try returns under catch', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const makePlan = plan.functionPlans.find((func) => func.name === 'make');
  const watPath = join(tempDirectory, 'wasm-gc-source-try-return-catch-finally.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-try-return-catch-finally.wasm');
...
Deno.test('compiler SourceHIR semantic lowering runs finally before try break under catch', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-try-break-catch-finally.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-try-break-catch-finally.wasm');
...
Deno.test('compiler SourceHIR semantic lowering runs finally before catch continue', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-catch-continue-finally.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-catch-continue-finally.wasm');
  ...
  const score = instance.instance.exports['main.ts:score'];
  assertEquals(typeof score, 'function');
  assertEquals((score as () => number)(), 33);
});
...
Deno.test('compiler SourceHIR semantic lowering runs nested finally before break completion', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-nested-try-finally-break.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-nested-try-finally-break.wasm');
  ...
  const score = instance.instance.exports['main.ts:score'];
  assertEquals(typeof score, 'function');
  assertEquals((score as () => number)(), 111);
});

Deno.test('compiler SourceHIR semantic lowering runs nested finally before continue completion', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-nested-try-finally-continue.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-nested-try-finally-continue.wasm');
  ...
  const score = instance.instance.exports['main.ts:score'];
  assertEquals(typeof score, 'function');
  assertEquals((score as () => number)(), 333);
});

Deno.test('compiler SourceHIR semantic lowering runs outer finally after nested catch break', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-nested-try-catch-finally-break.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-nested-try-catch-finally-break.wasm');
...
Deno.test('compiler SourceHIR semantic lowering runs outer finally after nested catch continue', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-nested-try-catch-finally-continue.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-nested-try-catch-finally-continue.wasm');
...
Deno.test('compiler SourceHIR semantic lowering runs finally before try break completion', async () => {
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-try-finally-break.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-try-finally-break.wasm');
...
Deno.test('compiler SourceHIR semantic lowering runs finally before try continue completion', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const plan = createWasmGcModulePlan(semantic, manifest);
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-try-finally-continue.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-try-finally-continue.wasm');
...
Deno.test('compiler SourceHIR semantic lowering emits runnable specialized object parameters', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
    ...
    },
    {
      path: 'main.ts',
      contents: `
        function sum(point: { left: number; right: number }): number {
          return point.left + point.right;
        }
  ...
  const sumPlan = plan.functionPlans.find((func) => func.name === 'sum');
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-object-param.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-object-param.wasm');
...
Deno.test('compiler SourceHIR semantic lowering emits runnable specialized object function results', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
    ...
    },
    {
      path: 'main.ts',
      contents: `
        function make(input: number): { left: number; right: number } {
          const point = { left: input, right: 7 };
          return point;
        }

        export function score(input: number): number {
          return make(input).left + make(input).right;
        }
      `,
    },
  ]);
  ...
  const makePlan = plan.functionPlans.find((func) => func.name === 'make');
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-object-result.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-object-result.wasm');
...
Deno.test('compiler SourceHIR semantic lowering emits runnable object result destructuring', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
  ...
  const makePlan = plan.functionPlans.find((func) => func.name === 'make');
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-object-result-binding.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-object-result-binding.wasm');
...
Deno.test('compiler SourceHIR semantic lowering emits runnable specialized object parameter mutation', async () => {
  const tempDirectory = await createTempProject([
    {
      path: 'tsconfig.json',
      contents: JSON.stringify({
    ...
    },
    {
      path: 'main.ts',
      contents: `
        function bump(point: { left: number; right: number }): number {
          point.left += 2;
          point.right = point.right + 3;
          return point.left + point.right;
        }
  ...
  const manifest = createRuntimeManifestFromSemanticModule(semantic);
  const plan = createWasmGcModulePlan(semantic, manifest);
  const bumpPlan = plan.functionPlans.find((func) => func.name === 'bump');
  const scorePlan = plan.functionPlans.find((func) => func.name === 'score');
  const watPath = join(tempDirectory, 'wasm-gc-source-object-param-mutation.wat');
  const wasmPath = join(tempDirectory, 'wasm-gc-source-object-param-mutation.wasm');

  assertEquals(
    manifest.familyRequirements.map((requirement) => requirement.family),
    ['specialized_object'],
  );
  assertEquals(bumpPlan?.bodyStatus, 'emittable');
  assertEquals(scorePlan?.bodyStatus, 'emittable');
  await Deno.writeTextFile(watPath, emitWasmGcModulePlan(plan));
  const result = await new Deno.Command('wasm-tools', {
    args: ['parse', watPath, '-o', wasmPath],
