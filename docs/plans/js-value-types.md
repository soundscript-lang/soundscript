...
This plan is intentionally JS-runtime-focused. It records how `#[value]` should behave on JS
targets now without changing the broader future-facing nominal-types note.
...
Generic shallow values work because runtime canonicalization keys off runtime values, not erased type
parameters.
