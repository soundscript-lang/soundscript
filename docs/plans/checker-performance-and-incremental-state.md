...
- project: a representative local mixed `.ts` / `.sts` workspace
- command:

```sh
SOUNDSCRIPT_CHECKER_TIMING=1 deno run --no-check --allow-env --allow-read --allow-run --allow-write src/main.ts check --project /path/to/project/tsconfig.soundscript.json
```
...
`ts.createSemanticDiagnosticsBuilderProgram(...)` in `createPreparedProgram(...)`, while keeping the
current external `PreparedProgram.program` surface intact.
