...
That means the checker should enforce the kept async model across `js-*` and `wasm-*` targets
alike rather than letting JS targets accept a broader async surface that later fails on wasm.
