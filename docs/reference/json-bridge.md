...
Use `JsonValue` when working with ordinary already-normalized JSON data. Use `JsonLikeValue` at the
boundary where soundscript needs to preserve or emit values that plain JSON does not model directly.
...
```ts
import { decodeJson, encodeJson, parseJsonLike, stringifyJsonLike } from 'sts:json';
```
