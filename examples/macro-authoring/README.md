# Macro Authoring Example

This example is the user-facing checked-in macro project.

It demonstrates two supported v1 authoring paths:

- `src/macro_demo.sts` shows stdlib macro usage through `Try(...)`.
- `src/user_macro_module.macro.sts` plus `src/user_macro_demo.sts` show import-scoped user-defined
  macros built on `sts:macros`.
...
- ordinary expanded TypeScript output for a `.sts` module rooted through an adjacent `.ts`
  entrypoint
...
Run the checked-in example test:

```sh
deno test --allow-env --allow-read --allow-write tests/integration/macro_authoring_example_test.ts
```
...
4. asserts the runtime behavior of both the nullish-`Try(...)` stdlib macro example and the
   user-defined macro example

## Notes

- Use `.sts` for macro-bearing source files.
- Real projects should install `@soundscript/soundscript` and use `moduleResolution: "Bundler"` or
...
- The repository `deno.json` maps `@soundscript/soundscript/runtime`,
  `@soundscript/soundscript/runtime/result`, `@soundscript/soundscript/runtime/match`, and
  `@soundscript/soundscript/runtime/errors` to the checked-in local stdlib sources so emitted
  example output can still run under Deno during repo tests.
