...
- The browser entry currently proves `createRoot(...)` plus shared routed component rendering. Full `hydrateRoot(...)` support on this real package path is still a separate gap.
- The next intended step is real browser-side mutation flow. The first attempt exposed two real generic gaps: JSX child-array export in SSR (`generic_owned_heap_array_to_host_array`) and callback-driven browser wrapper rendering on the `createRoot(...)` path.
- After those gaps are fixed, the next intended step is to swap the in-memory todo creation path for Sequelize.
